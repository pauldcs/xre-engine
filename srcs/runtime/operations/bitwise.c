...
static frame_block_t *bitwise_lshift_op(frame_block_t *lv, frame_block_t *rv) {
  if (rv->_data.value < 0) {
    frame_block_free(&lv);
    frame_block_free(&rv);
    return error_block_alloc(XRE_VALUE_ERROR, XRE_NEGATIVE_SHIFT_ERROR);
  }

  if (rv->_data.value > 64) {
    frame_block_free(&lv);
    frame_block_free(&rv);
    return error_block_alloc(XRE_VALUE_ERROR, XRE_EXCEEDS_SHIFT_LIMIT_ERROR);
  }

  return value_block_alloc(lv->_data.value << rv->_data.value);
}
