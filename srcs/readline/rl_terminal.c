...
rl_init_termcaps(void) {
  ...
  __return_val_if_fail__(term_type &&
    "TERM must be set (see 'env')",
    false
  );

  ret = tgetent(NULL, term_type);
  __return_val_if_fail__(ret != -1 && 
...
void
rl_get_terminal_caps(void) {

  static char __[2048];
  for (size_t i = 0; i < NUM_TC_STRINGS; i++) {
    char **_ptr = (char **)&__;
    *(tc_strings[i].tc_value) = tgetstr (
      (char *)tc_strings[i].tc_var,
      _ptr
    );

    if (*(tc_strings[i].tc_value)) {
      tc_strings[i].tc_len = strlen(*(tc_strings[i].tc_value));
    } /* else {
      fprintf(stderr,
...
bool
rl_get_window_size(size_t *rows, size_t *cols) {
...
rl_at_exit_disable_raw_mode(void) {
