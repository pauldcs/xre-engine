...
static bool accept_token(array_t *tokens, size_t len) {
...
bool
xre_expr_lex (const char *expr, array_t *tokens) {
...
not_a_constant_value:
      ...
      switch (*ptr) {
        case '*':
          if   (*(ptr + 1) == '=')
            { _token._kind = __MUL_ASSIGN__; tf = 2; }
          else 
            { _token._kind = __MUL__; tf = 1; }
        
        break;
        case '/':
          if   (*(ptr + 1) == '=')
            { _token._kind = __DIV_ASSIGN__; tf = 2; }
          else 
            { _token._kind = __DIV__; tf = 1; }
        
        break;
        case '%':
          if   (*(ptr + 1) == '=')
            { _token._kind = __MOD_ASSIGN__; tf = 2; }
          else 
            { _token._kind = __MOD__; tf = 1; }
        
        break;
        case '(': _token._kind = __LPAREN__; tf = 1;
        
        break;
        case ')': _token._kind = __RPAREN__; tf = 1;
        ...
        case '=':
          if   (*(ptr + 1) == '=')
            { _token._kind = __EQ__; tf = 2; }
          else 
            { _token._kind = __ASSIGN__; tf = 1; }
        
        break;
        case '!':
          if   (*(ptr + 1) == '=')
            { _token._kind = __NE__; tf = 2; }
          else
            { _token._kind = __NOT__; tf = 1; }
        
        break;
        case '+':
          if (*(ptr + 1) == '=')
            { _token._kind = __ADD_ASSIGN__; tf = 2; }
          else 
            { _token._kind = __ADD__; tf = 1; }
        
        break;
        case '-':
          if (*(ptr + 1) == '=')
            { _token._kind = __SUB_ASSIGN__; tf = 2; }
          else 
            { _token._kind = __SUB__; tf = 1; }
        
        break;
        case '&':
          if (*(ptr + 1) == '&')
            { _token._kind = __AND__; tf = 2; }
          else 
            { _token._kind = __BAND__; tf = 1; }
        
        break;
        case '|':
          if (*(ptr + 1) == '|')
            { _token._kind = __OR__; tf = 2; }
          else 
            { _token._kind = __BOR__; tf = 1; }
        
        break;
        case '^':
          if (*(ptr + 1) == '^') 
            { _token._kind = __POW__; tf = 2; }
          else 
            { _token._kind = __BXOR__; tf = 1; }
        ...
        case '<':
          if      (*(ptr + 1) == '<') 
            ...
            { _token._kind = __LT__; tf = 1; }
        
        break;
        case '>':
          if      (*(ptr + 1) == '>')
            { _token._kind = __RSHIFT__; tf = 2; }
          else if (*(ptr + 1) == '=')
            { _token._kind = __GE__; tf = 2; }
          else 
            { _token._kind = __GT__; tf = 1; }
