...
bool xre_expr_lex(const char *expr, array_t *tokens)
{
	...
	while (*ptr) {
		...
		if (isdigit(*ptr) || *ptr == '-') {
			...
			if (!tmp) {
				lexer_error_g.class =
					error_type_to_class(XRE_OVERFLOW_ERROR);
