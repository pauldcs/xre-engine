...
void *filename_stack_pop(void) {
	if (__top > 0) return ((void *)__stack[__top--]);
	return (NULL);
}
 ...
 *         + the function should not be allowed to take additional arguments if the stack
 *         is not empty (only 'c' could take an additional argument)
 */
