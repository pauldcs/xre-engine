...
err_notif_t	   _error;
struct statements *__statements__ = NULL;

typedef bool (*runtime_op_t)(struct statements *);
...
static bool
alloc_zeroed_stmt_tree(size_t size, struct statements **buffer)
{
	size_t alloc_size = sizeof(struct statements) * size;
...
)
{
	...
	struct statements *current	 = stmt + index;
	int		   n		 = 0;
	int		   tmp		 = 0;
...
void stmt_tree_destroy(struct statements *tree)
{
	free(tree);
}

static void
inner(struct statements *stmt,
      size_t		 depth,
      vec_t		*parent_vec,
      vec_t		*cache,
...
{
	inner(&__statements__[stmt->br.left],
	      depth + 1,
	      parent_vec,
	      cache,
	      is_scope_change,
	      ret);
	inner(&__statements__[stmt->br.right],
	      depth + 1,
	      parent_vec,
	      cache,
	      is_scope_modifier_kind(stmt->orig->_kind),
	      ret);
}
...
static ssize_t is_cached(const char *str, vec_t *cache)
{
...
static void
inner2(struct statements *stmt,
       size_t		  depth,
       vec_t		 *cache,
       size_t		  vars);

static void put_binop2(
	struct statements *stmt,
	size_t		   depth,
	vec_t		  *cache,
	size_t		   vars
...
)
{
	...
	while (c--) {
		s = (statement_name_t *)vec_access(stmt->children, c);
		inner2(&__statements__[*s], depth + 1, cache, vars);
	}
}

static void put_uniop2(
	struct statements *stmt,
	size_t		   depth,
	vec_t		  *cache,
	size_t		   vars
       ...
       size_t		  vars)
{
	...
	if (stmt->orig->_type == EXPR_TYPE_VALUE) {
		printf("  &__var_%02lld__\n", stmt->value);
	} else {
		printf("[%s] %s (depth: %02zu, %zu:%zu)\n",
		       expr_type_to_string(stmt->orig->_type),
		       expr_kind_to_string(stmt->orig->_kind),
		       stmt->orig->_depth,
		       stmt->orig->_line,
		       stmt->orig->_cols);
	}
...
void scoper(struct statements *tree)
{
