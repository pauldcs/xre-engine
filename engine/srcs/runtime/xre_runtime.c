...
static int
stmt_tree_init(ast_stmt_t *stmt, xre_ast_t *ast, bool reset_index)
{
	static int index = 0;
	if (reset_index) {
		index = 0;
	...
	case __VARIABLE__:
		current->value =
			symtab_entry_create((char *)ast->string);
		break;
...
static bool stmt_tree_create(xre_ast_t *ast, ast_stmt_t **buffer)
{
	if (!alloc_zeroed_stmt_tree(
		    get_tree_size(ast) * sizeof(ast_stmt_t), buffer
	    )) {
		return (false);
	}
