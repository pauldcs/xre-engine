...
static char *
history_get_index(const history_vec_t *history, int index)
{
	if (index < (int)vec_size(history)) {
		return (*(char **)vec_at(history, index));
	}
	return (NULL);
}

static void
history_move_cursor(const history_vec_t *history, int depth)
{
...
bool handle_line_mode_char(int ch, const history_vec_t *history)
{
	...
	case RL_KEY_DOWN:
		history_depth = rl_clamp(
			history_depth - 1, 0, vec_size(history) - 1
		);
		history_move_cursor(history, history_depth);
